...
def find_consecutive_index(array)
  ...
  while i < (array.length - 1)
    current_num = array[i]
    next_num = array[i + 1]
    difference = (next_num - current_num).abs
    
    return i if difference == 1
    i += 1
  end
...
def consecutive_collapse(array)
  current_array = array
  while true
    index = find_consecutive_index(current_array)

    if index == -1
      return current_array
    end

    prefix = current_array[0...index]
    suffix = current_array[(index + 2)..-1]
    current_array = prefix + suffix
  end
end

def consecutive_collapse(array)
  index = find_consecutive_index(array)
  return array if index == -1

  prefix = array[0...index]
  suffix = array[(index + 2)..-1]
  current_array = prefix + suffix

  consecutive_collapse(current_array)
end
