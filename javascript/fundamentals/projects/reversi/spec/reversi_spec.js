...
describe("Board", function () {
  ...
  describe("#_positionsToFlip", function () {
    it("returns empty array when pos is not on the board", function () {
      expect(
        testBoard._positionsToFlip([-1, -1], "white", [1, 0]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([9, 9], "white", [1, 0]).length
      ).toEqual(0);
    });

    it("returns empty array when there is a blank space one position away from the current position", function () {
      expect(
        testBoard._positionsToFlip([4, 6], "white", [0, 1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([4, 6], "white", [-1, 1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([4, 6], "white", [-1, 0]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([4, 6], "white", [0, -1]).length
      ).toEqual(0);
    ...
    it("returns empty array if no pieces of the opposite color are found", function () {
      ...
      expect(
        testBoard._positionsToFlip([4, 5], "white", [0, -1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([5, 4], "white", [-1, 0]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([5, 5], "white", [-1, -1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([3, 2], "white", [0, 1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([2, 2], "white", [1, 1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([2, 3], "white", [1, 0]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([7, 1], "white", [-1, 1]).length
      ).toEqual(0);
      expect(
        testBoard._positionsToFlip([5, 3], "white", [1, -1]).length
      ).toEqual(0);
    });
    ...
    it("returns positions for longer horizontal and vertical cases ", function () {
      ...
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([5, 7], "white", [-1, 0])
        )
      ).toEqual(
        JSON.stringify([
          [4, 7],
      ...
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([6, 1], "black", [0, 1])
        )
      ).toEqual(
        JSON.stringify([
          [6, 2],
      ...
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([7, 6], "black", [0, -1])
        )
      ).toEqual(
        JSON.stringify([
          [7, 5],
    ...
    it("returns positions for longer diagonal cases", function () {
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([5, 2], "white", [-1, 1])
        )
      ).toEqual(
        JSON.stringify([
          [4, 3],
      ...
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([5, 5], "black", [-1, -1])
        )
      ).toEqual(
        JSON.stringify([
          [4, 4],
      ...
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([0, 2], "white", [1, 1])
        )
      ).toEqual(
        JSON.stringify([
          [1, 3],
      ...
      expect(
        JSON.stringify(
          testBoardLongHorzDiagonal._positionsToFlip([0, 5], "black", [1, -1])
        )
      ).toEqual(
        JSON.stringify([
          [1, 4],
